...
<div v-else><a target="_blank" :href="'interact.html?address=' + this.result.BOPAddress">WP created!</a></div>
`
});
...
Vue.component('bop-event-row', {
  ...
  template:
`
<div v-if="this.event.event == 'Created'" align='center'><blocknum-output :blocknum='event.blockNumber' :timestamp='event.timestamp'></blocknum-output><br><div class='well well-sm' align='left' style='background-color:#dddddd;display:inline-block'>WP created.</div></div>
...
<div v-else-if="this.event.event == 'FundsRecovered'" align='left'><blocknum-output :blocknum='event.blockNumber' :timestamp='event.timestamp'></blocknum-output><br><div class='well well-sm' align='left' style='background-color:#ff8888;display:inline-block'>Payer cancelled the WP and recovered the funds.</div></div>
<div v-else-if="this.event.event == 'Committed'" align='center'><blocknum-output :blocknum='event.blockNumber' :timestamp='event.timestamp'></blocknum-output><br><div class='well well-sm' align='left' style='background-color:#ccffcc;display:inline-block'>Worker committed to the WP.</div></div>
